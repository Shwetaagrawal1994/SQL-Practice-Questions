...
# ################################ APPLY FUNCTION ########################################
print(">"*30, "APPLY FUNCTION", ">"*30)
df = pd.DataFrame([[9, 25]] * 3, columns=['P', 'Q'])
print(df)

# ---- Square root all rows
print(df.apply(np.sqrt) ) # apply function with default axis = 0 (index axis)
# ---- Square root all columns
...
# # Lambda with filter()
lst = [33, 3, 22, 2, 11, 1]
print(filter(lambda x: x > 10, lst)) # NOTE - will return <filter object at 0x1056efb20>
print(list(filter(lambda x: x > 10, lst))) # NOTE - will return [33, 22, 11]
